...
def input_from_blif(
    blif, block=None, merge_io_vectors=True, clock_name="clk", top_model=None
):
...
def input_from_verilog(
    verilog, clock_name="clk", toplevel=None, leave_in_dir=None, block=None
):
...
class _VerilogSanitizer(_NameSanitizer):
    ...
    _verilog_reserved = """always and assign automatic begin buf bufif0 bufif1 case casex casez cell cmos
    ...
    def __init__(self, internal_prefix="_sani_temp", map_valid_vals=True):
        ...
        super(_VerilogSanitizer, self).__init__(
            self._ver_regex, internal_prefix, map_valid_vals, self._extra_checks
        )

    def _extra_checks(self, str):
        return (
            str not in self._verilog_reserved_set and str != "clk" and len(str) <= 1024
        )  # is not a Verilog reserved keyword  # not the clock signal  # not too long to be a Verilog id
...
def _to_verilog_header(file, block, varname, add_reset):
    ...
    for w in name_sorted(registers):
        print(
            "    reg{:s} {:s};".format(_verilog_vector_decl(w), varname(w)), file=file
        )
    ...
    for m in sorted(memories, key=lambda m: m.id):
        ...
        for net in _net_sorted(block.logic_subset("m"), varname):
            if net.op_param[1] == m:
                ...
                print(
                    "    assign {:s} = mem_{}[{:s}];".format(dest, m_id, index),
                    file=file,
                )
        print("", file=file)
...
def output_to_reticle(open_file, block=None):
    ...
    def emit_var_tuple(vars):
        if isinstance(vars, list) | isinstance(vars, set):
            res = []
            for v in vars:
                res.append(emit_var(v))
            return res
        else:
            raise PyrtlInternalError(
                "reticle emit_var_tuple(), {} must be a set or list".format(vars)
            )

    def emit_io(ioset):
        if isinstance(ioset, set):
            io = _name_sorted(ioset)
            io = emit_var_tuple(io)
            io = ", ".join(io)
            return "({})".format(io)
        else:
            raise PyrtlInternalError(
    ...
    # emit signature
    inputs = emit_io(block.wirevector_subset(Input))
    ...
    for log_net in _net_sorted(block.logic_subset()):
        ...
        if log_net.op == "s" and isinstance(log_net.args[0], Const) and log_net.args[0].bitwidth == 1 and len(log_net.op_param) == 1:
            ...
            f.write("{}{} = id({});\n".format(indent, out, arg_name))
        elif log_net.op == "w":
            f.write("{}{} = id({});\n".format(indent, dst, arg_name))
        elif log_net.op == "r":
            f.write("{}{} = reg[0]({}, {});\n".format(indent, dst, arg_name, const_t))
    f.write("}")
    return 0
